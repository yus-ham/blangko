...
async function insert(data) {
    ...
    const res = common.db().raw(`insert into ${table} (user_id,client_id,value,expired_at) values(${data.user_id||0},'${data.client_id}','${value}',${expired_at})`)
    return res.then(r => {
        return {
            user_id: data[0],
            client_id: data[1],
...
export default {
    async getOrCreate({ user_id, client_id }) {
        const token = await common.db().raw(`select * from ${table} where user_id=? and client_id=? limit 1`, [user_id, client_id])
